...
Indicates whether the list contains any of the values from another list. Returns true if the values are found in the list, false otherwise. list: The list to search. values: The list of values to search for in the first list. equationCriteria: (Optional) The comparer used to determine if two values are equal.
...
```m
List.ContainsAny({"dog", "cat", "racoon", "horse", "rabbit"}, {"Horse", "OWL"},
