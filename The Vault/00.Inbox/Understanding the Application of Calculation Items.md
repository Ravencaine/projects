...
created: 2026-08-04
description: "This article explains how calculation items are applied to measure references, and it is part of a series dedicated to calculation groups in DAX. Before sta"
...
In order to apply the calculation item in an expression, you need to filter the calculation group. You can create this filter using [CALCULATE](https://dax.guide/calculate/?aff=sqlbi), like in the following example; this is the same technique used by Power BI when using slicers and visuals:
...
When [CALCULATE](https://dax.guide/calculate/?aff=sqlbi) applies a filter to a calculation item, DAX uses the definition of the calculation item to rewrite the expression before evaluating it. Therefore, based on the definition of the calculation item, the previous code is interpreted as follows:
...
In the previous example, the calculation item does not perform any transformation because the code inside [CALCULATE](https://dax.guide/calculate/?aff=sqlbi) does not use any measure. The following code is the one executed after the application of the calculation item:
...
If the expression inside [CALCULATE](https://dax.guide/calculate/?aff=sqlbi) contains multiple measure references, all of them are replaced with the calculation item definition. For example, the expression in the following *Cost Ratio YTD* measure contains two measure references, *Total Cost* and *Sales Amount*:
...
![](99.System/Attachments/Calculation-groups-3-Advanced-Application-01.png)

Nevertheless, you must be very careful because the *CR YTD Simplified* measure does not correspond to the actual code generated by the calculation item, which is the code in *CR YTD Actual Code*. In this very special case, the two versions are equivalent. However, in more complex scenarios the difference is significant, and such a large difference can lead to unintended results that are extremely hard to follow and understand. Let us analyze a couple of examples. In the first example the *Sales YTD 2008 2009* measure has two nested [CALCULATE](https://dax.guide/calculate/?aff=sqlbi) functions: the outer [CALCULATE](https://dax.guide/calculate/?aff=sqlbi) sets a filter on the year 2008, whereas the inner [CALCULATE](https://dax.guide/calculate/?aff=sqlbi) sets a filter on the year 2009:
...
The outer [CALCULATE](https://dax.guide/calculate/?aff=sqlbi) filters the calculation item to the **YTD** value. Nevertheless, the application of the calculation item does not change the expression because the expression does not directly contain any measure. [CALCULATE](https://dax.guide/calculate/?aff=sqlbi) filters the calculation item, but its application does not lead to any modifications to the code.

Pay attention to the fact that the Sales Amount measure is within the scope of the inner [CALCULATE](https://dax.guide/calculate/?aff=sqlbi). The application of a calculation item modifies the measures in the current scope of the filter context; it does not affect nested filter context scopes. Those are handled by their own [CALCULATE](https://dax.guide/calculate/?aff=sqlbi) or equivalent code, such as [CALCULATETABLE](https://dax.guide/calculatetable/?aff=sqlbi) or context transitions which may or may not retain the same filter on the calculation item.

When the inner [CALCULATE](https://dax.guide/calculate/?aff=sqlbi) applies its filter context, it does not change the filter status of the calculation item. Therefore, the engine finds that the calculation item is still filtered, and it remains filtered if no other [CALCULATE](https://dax.guide/calculate/?aff=sqlbi) changes it. Same as if it were a regular column. The inner [CALCULATE](https://dax.guide/calculate/?aff=sqlbi) contains a measure reference, and DAX performs the application of the calculation item. The resulting code corresponds to the definition of the *Sales YTD 2008 2009 Actual Code* measure:

```
Sales YTD 20082009Actual Code :=
CALCULATE(
    CALCULATE(
        CALCULATE(
            [Sales Amount],
...
![](99.System/Attachments/Calculation-groups-3-Advanced-Application-02.png)
...
The [DATESYTD](https://dax.guide/datesytd/?aff=sqlbi) function is applied when the filter context is filtering the year 2009, not 2008. Despite the calculation item being filtered along with the filter for the year 2008, its actual application took place in a different filter context, namely the inner filter context. The behavior is counter-intuitive to say the least. The more complex the expression used inside [CALCULATE](https://dax.guide/calculate/?aff=sqlbi), the harder it becomes to understand how the application works.
...
If this expression is executed within an outer [CALCULATE](https://dax.guide/calculate/?aff=sqlbi) that changes the calculation to a **YTD**, the result is the following new formula that produces an unexpected result:
...
```
Sales WD YTD 2008Expected Code :=
CALCULATE(
    CALCULATE(
        DIVIDE(
            [Sales Amount],-- Measure reference
...
You can easily avoid this complexity by obeying the best practice: Use [CALCULATE](https://dax.guide/calculate/?aff=sqlbi) with calculation items only to invoke an individual measure. When one authors the *Sales WD YTD 2008 Fixed* measure that includes the full expression and uses the *Sales WD* measure in a single [CALCULATE](https://dax.guide/calculate/?aff=sqlbi) function, the code is very different and easier to use:
...
```
Sales WD YTD 2008Fixed Actual Code :=
CALCULATE(
    CALCULATE(
        [Sales WD],
        DATESYTD('Date'[Date])
...
In this latter example the filter provided by [DATESYTD](https://dax.guide/datesytd/?aff=sqlbi) surrounds the entire expression, leading to the code that one intuitively expects from the application of the calculation item. The result of the *Sales WD YTD 2008 Fixed* and *Sales WD YTD 2008 Fixed Actual Code* measures is visible in the previous screenshot.
