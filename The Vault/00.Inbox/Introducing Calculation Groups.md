...
created: 2026-08-04
description: "This article is the first of a series dedicated to calculation groups in DAX. This introduction explains the capabilities of this feature and how to create"
...
All the previous measures only differ in their base measure; they all apply the same [DATESYTD](https://dax.guide/datesytd/?aff=sqlbi) filter context to different base measures. It would be great if a developer were given the opportunity to define a more generic calculation, using a placeholder for the measure:
...
![](99.System/Attachments/Calculation-groups-1-Introduction-01.png)
...
![](99.System/Attachments/Calculation-groups-1-Introduction-02.png)
...
![](99.System/Attachments/Calculation-groups-1-Introduction-04.png)

If on the same report the user selects **SPLY**, the result will be very different.

![](99.System/Attachments/Calculation-groups-1-Introduction-05.png)
...
![](99.System/Attachments/Calculation-groups-1-Introduction-06.png)
...
![](99.System/Attachments/Calculation-groups-1-Introduction-07.png)
...
![](99.System/Attachments/Calculation-groups-1-Introduction-08.png)
...
![](99.System/Attachments/Calculation-groups-1-Introduction-09.png)
...
These calculation items are not modifiers of the original measure. Instead, they completely replace the original measure with a new one. To obtain this behavior, we omitted a reference to [SELECTEDMEASURE](https://dax.guide/selectedmeasure/?aff=sqlbi) in the expression. [SELECTEDMEASURE](https://dax.guide/selectedmeasure/?aff=sqlbi) is used very often in calculation items, but it is not mandatory.
...
One can write conditions in DAX that analyze the current measure evaluated in the model by using either [ISSELECTEDMEASURE](https://dax.guide/isselectedmeasure/?aff=sqlbi) or [SELECTEDMEASURENAME](https://dax.guide/selectedmeasurename/?aff=sqlbi). The [ISSELECTEDMEASURE](https://dax.guide/isselectedmeasure/?aff=sqlbi) function returns TRUE if the measure evaluated by [SELECTEDMEASURE](https://dax.guide/selectedmeasure/?aff=sqlbi) is included in the list of measures specified in the arguments. For example, the following code applies the calculation item to any measure, except to the *Margin %* measure:

```
IF(
    NOTISSELECTEDMEASURE([Margin %]),
    DIVIDE(
        SELECTEDMEASURE(),
        COUNTROWS('Date')
    )
...
Another function that can be used to analyze the selected measure in a calculation item expression is [SELECTEDMEASURENAME](https://dax.guide/selectedmeasurename/?aff=sqlbi), which returns a string instead of a Boolean value. For example, the previous code can be written also this way:
...
[ISSELECTEDMEASURE](https://dax.guide/isselectedmeasure/?aff=sqlbi) is preferable over [SELECTEDMEASURENAME](https://dax.guide/selectedmeasurename/?aff=sqlbi) for several reasons:

- If the measure name is misspelled using a comparison with [SELECTEDMEASURENAME](https://dax.guide/selectedmeasurename/?aff=sqlbi), the DAX code simply return FALSE without raising an error.
- If the measure name is misspelled using [ISSELECTEDMEASURE](https://dax.guide/isselectedmeasure/?aff=sqlbi), the expression fails with the error Invalid input arguments for [ISSELECTEDMEASURE](https://dax.guide/isselectedmeasure/?aff=sqlbi).
- If a measure is renamed in the model, all the expressions using [ISSELECTEDMEASURE](https://dax.guide/isselectedmeasure/?aff=sqlbi) are automatically renamed in the model editor (formula fixup), whereas the strings compared to [SELECTEDMEASURENAME](https://dax.guide/selectedmeasurename/?aff=sqlbi) must be updated manually.

The [SELECTEDMEASURENAME](https://dax.guide/selectedmeasurename/?aff=sqlbi) function should be considered when the business logic of a calculation item must apply a transformation based on an external configuration. For example, the function might be useful when there is a table with a list of measures that should enable a behavior in a calculation item so that the model has an external configuration that can be modified without requiring an update of the DAX code.
