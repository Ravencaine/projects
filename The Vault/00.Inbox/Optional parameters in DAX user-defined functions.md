...
created: 2026-08-04
description: "This article describes how to define optional parameters in DAX user-defined functions and set default values for parameters not specified by the caller. Wh"
...
```
DEFINE
    FUNCTIONIncrement =(x : NUMERIC,y : NUMERIC=1)=> x +y
EVALUATE
{
...
The first call provides only *x*: *y* defaults to 1 and *limit* defaults to 10, so the result is *[MIN](https://dax.guide/min/?aff=sqlbi) ( 1 + 1, 10 )*, which is 2. The second call provides *x* and *y* but omits *limit*, so the result is *[MIN](https://dax.guide/min/?aff=sqlbi) ( 5 + 4, 10 )*, which is 9. The third call provides all three arguments, so the result is *[MIN](https://dax.guide/min/?aff=sqlbi) ( 5 + 25, 20 )*, which is 20.
...
In the second call, the empty position before the comma tells DAX to use the default expression of *y*. The value 20 is assigned to *limit*. The result is therefore *[MIN](https://dax.guide/min/?aff=sqlbi) ( 5 + 1, 20 )*, which is 6.
...
```
DEFINE
    FUNCTIONRoundDivision =
        (
            x : NUMERIC,
            y : NUMERIC,
            roundingDigits =BLANK()
        )=>
            VARResult =DIVIDE(x,y )
            RETURN
                IF(
...
The first call omits *roundingDigits*. Its default expression, [BLANK](https://dax.guide/blank/?aff=sqlbi), becomes the value of the parameter, so [ISBLANK](https://dax.guide/isblank/?aff=sqlbi) returns TRUE and the function returns the unrounded result. The other calls provide the number of digits, so [ISBLANK](https://dax.guide/isblank/?aff=sqlbi) returns FALSE and the function rounds the result accordingly: zero digits give 1, one digit gives 0.7, two digits give 0.67, and three digits give 0.667.

This pattern is useful when the choice is between doing something and doing nothing. By using [BLANK](https://dax.guide/blank/?aff=sqlbi) as the default value, we represent the absence of a value, which the function then interprets.

Be mindful of one limitation: the function cannot distinguish an omitted argument from an argument that is explicitly blank. If the caller writes *RoundDivision ( 2, 3, [BLANK](https://dax.guide/blank/?aff=sqlbi) () )*, [ISBLANK](https://dax.guide/isblank/?aff=sqlbi) returns TRUE exactly as it does for an omitted argument. This is rarely a problem in practice, but it is worth keeping in mind when blank is a legitimate value for the parameter.
