...
created: 2026-08-04
description: "This article describes how to use calculation groups to dynamically replace only a partial expression in a complex DAX calculation. This article contains a"
...
If your requirement is to modify the *Sales Forecast* measure through a slicer selection, you cannot use a calculation group to define different implementations of *Sales Forecast*. You should rewrite the [DIVIDE](https://dax.guide/divide/?aff=sqlbi) expressions in the calculation group because [SELECTEDMEASURE](https://dax.guide/selectedmeasure/?aff=sqlbi) corresponds to *Daily Sales* or *Monthly Sales*.
...
![](99.System/Attachments/Using-calculation-groups-to-selectively-replace-measures-in-DAX-expressions-01.png)
...
![](99.System/Attachments/Using-calculation-groups-to-selectively-replace-measures-in-DAX-expressions-02.png)

We need a calculation group with two calculation items. When the user selects *Net Price*, we replace *Sales Amount* with the formula using *Net Price*. Alternatively, when the user selects *Unit Price*, we use the formula with *Unit Price*. The calculation item needs to replace the formula. Therefore, it does not use [SELECTEDMEASURE](https://dax.guide/selectedmeasure/?aff=sqlbi); it merely replaces the entire expression:
...
![](99.System/Attachments/Using-calculation-groups-to-selectively-replace-measures-in-DAX-expressions-03.png)
...
We want to tell the calculation item that it needs to affect only *Sales Amount*, while keeping the other measures untouched. DAX offers the [ISSELECTEDMEASURE](https://dax.guide/isselectedmeasure/?aff=sqlbi) function to check the name of the selected measure. It looks as though an [IF](https://dax.guide/if/?aff=sqlbi) statement is all we need:
...
![](99.System/Attachments/Using-calculation-groups-to-selectively-replace-measures-in-DAX-expressions-04.png)

This is to be expected. During the execution of the [IF](https://dax.guide/if/?aff=sqlbi) functions, if any measure other than *Sales Amount* is the one selected then the else branch returns blank. As usual DAX is doing what we are asking it to do, and it is not doing what we want it to do… Regardless, it looks like we only need to fix the else branch of both [IF](https://dax.guide/if/?aff=sqlbi) functions by returning [SELECTEDMEASURE](https://dax.guide/selectedmeasure/?aff=sqlbi). This way, it replaces only *Sales Amount**,* and it should not replace any other measure:
...
![](99.System/Attachments/Using-calculation-groups-to-selectively-replace-measures-in-DAX-expressions-05.png)
...
The calculation item is applied to the three measures: *Total Cost*, *Margin %*, and *Sales Amount*. Each time, the corresponding measure is returned by [SELECTEDMEASURE](https://dax.guide/selectedmeasure/?aff=sqlbi).
...
When the calculation item is applied to *Total Cost*, it applies [SELECTEDMEASURE](https://dax.guide/selectedmeasure/?aff=sqlbi) which is the original measure: *Total Cost*. Still fine.

The issue is with *Margin %*. When the calculation item is applied to *Margin %*, the behavior is the very same as the one described for *Total Cost*. Because the [SELECTEDMEASURE](https://dax.guide/selectedmeasure/?aff=sqlbi) is not *Sales Amount*, the calculation item transforms the selected measure in the selected measure itself, not changing its result.
...
This is the important thing about calculation groups: when you define a calculation group and you protect it with an [IF](https://dax.guide/if/?aff=sqlbi) statement that checks the result of [ISSELECTEDMEASURE](https://dax.guide/isselectedmeasure/?aff=sqlbi), you are not telling the engine that the calculation item must be applied to only that measure. You are modifying the application of the calculation item to any measure, choosing different expansions based on which measure you are replacing. The reference to *Sales Amount* inside *Margin %* remains untouched. Because the default implementation of *Sales Amount* uses *Net Price*, *Margin %* is not changing its behavior based on the calculation item: it is always using the default version of the measure.
...
![](99.System/Attachments/Using-calculation-groups-to-selectively-replace-measures-in-DAX-expressions-07.png)

We can force the application of a calculation item to a specific part of our calculation by using [CALCULATE](https://dax.guide/calculate/?aff=sqlbi) to filter the calculation group. This [CALCULATE](https://dax.guide/calculate/?aff=sqlbi) needs to read the desired value from a table accessible to the user.
...
- We add [CALCULATE](https://dax.guide/calculate/?aff=sqlbi) around the *\[Sales Amount\]* measure reference. It reads the content of the disconnected table and uses it to force the application of the calculation item exactly where we want it to happen.

Here is an excerpt of the final model below.

![](99.System/Attachments/Using-calculation-groups-to-selectively-replace-measures-in-DAX-expressions-08.png)
...
The last step requires adding [CALCULATE](https://dax.guide/calculate/?aff=sqlbi) around the *Sales Amount* reference. To achieve this, we create a new *Internal* *Sales Amount* hidden measure that returns the default value of *Sales Amount*. In the *Sales Amount* measure, we use [CALCULATE](https://dax.guide/calculate/?aff=sqlbi) to invoke the *Internal* *Sales Amount* measure by applying the required calculation item in the filter context. We choose the calculation item to use by reading the selected value in the parameter table:
...
By embedding [CALCULATE](https://dax.guide/calculate/?aff=sqlbi) in *Sales Amount*, there is no need to change the code of *Margin %*. Whenever a measure calls *Sales Amount*, the measure takes care of reading the required price to use from the parameter table and it applies the calculation item accordingly. The calculation item application does not take place in the query; it takes place in the measure. Exactly where we wanted it to take place.
...
![](99.System/Attachments/Using-calculation-groups-to-selectively-replace-measures-in-DAX-expressions-09.png)

You can consider this example as a pattern to inject any parameter from the report inside your measures, by using calculation groups instead of [IF](https://dax.guide/if/?aff=sqlbi) statements – as we were used to doing before the advent of calculation groups.

Indeed, we could solve the same scenario using [SWITCH](https://dax.guide/switch/?aff=sqlbi) or [IF](https://dax.guide/if/?aff=sqlbi) statements without calculation groups at all. By using the calculation item, we move the definition of the expression to use into a different part of the model, which is easy to maintain and extend. By using a [SWITCH](https://dax.guide/switch/?aff=sqlbi) or [IF](https://dax.guide/if/?aff=sqlbi) statement in *Sales Amount* we could implement a similar logic, but any change would affect the code in *Sales Amount*. The main motivation to use calculation groups is the maintainability of the solution. From a performance point of view, the results could be different and related to other details in the model. For larger and more complex models, the calculation groups approach could also provide better performance – although preparing a benchmark is always critical.
