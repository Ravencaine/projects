...
created: 2026-08-04
description: "This article introduces the new calendar-based time intelligence functions in DAX, available in preview from the September 2025 release of Power BI. Since i"
...
![](99.System/Attachments/image1-119-scaled.png)
...
![](99.System/Attachments/image3-103.png)
...
So far, [SAMEPERIODLASTYEAR](https://dax.guide/sameperiodlastyear/?aff=sqlbi) has been able to identify the target filter: February 2019. However, applying the new filter is not enough, because it would conflict with the existing filter context. Therefore, [CALCULATE](https://dax.guide/calculate/?aff=sqlbi) must remove some filters from the *Date* table. If this were a classic function, it would use a ruthless solution: use [REMOVEFILTERS](https://dax.guide/removefilters/?aff=sqlbi) over the entire *Date* table. However, we have already discussed that the automatic [REMOVEFILTERS](https://dax.guide/removefilters/?aff=sqlbi) poses serious issues for the handling of filter-keep columns.
...
In our example, the new filter context generated by [SAMEPERIODLASTYEAR](https://dax.guide/sameperiodlastyear/?aff=sqlbi) contains two columns: *Date\[Month\] = February* and *Date\[Year\] = 2019*, which are columns tagged as *Month of Year* and *Year* category, respectively. When the calendar-based time intelligence functions evaluate the filter context for each “X” column tagged with a category in the calendar, here is what happens:
...
- Once the dependencies are identified, [REMOVEFILTERS](https://dax.guide/removefilters/?aff=sqlbi) is applied to all columns above that are in the same vertical position as either *X* or a dependency.

The following illustration shows a graphical representation of this behavior, which shows the underlying [DATEADD](https://dax.guide/dateadd/?aff=sqlbi) function invoked by [SAMEPERIODLASTYEAR](https://dax.guide/sameperiodlastyear/?aff=sqlbi) in the equivalent DAX code (more on this equivalency later in the chapter). The filters on *Week of Month*, *Day of Month*, and *Day of Week* are not altered, even if present, as those categories represent time-related columns. They are not cleared in this scenario, since they are filter-keep columns.

![](99.System/Attachments/image4-97-scaled.png)

Filter clearing works across calendars. If the table includes multiple calendars, the filter is removed from any column tagged in any calendar at the levels where the filter clearing happens. In other words, if the *Date* table includes an ISO calendar with an *ISO Month* column tagged as *Month of Year*, the *ISO Month* will receive [REMOVEFILTERS](https://dax.guide/removefilters/?aff=sqlbi) in this example, even though it does not belong to the current calendar. This is the reason why creating multiple calendars on the same *Date* table can be confusing. It is possible to create multiple calendars, but developers need to pay special attention to which filters will be removed and which ones will be maintained.

### Understanding lateral shift and hierarchical shift

So far in this chapter, we analyzed two functions: [SAMEPERIODLASTYEAR](https://dax.guide/sameperiodlastyear/?aff=sqlbi) and [DATESYTD](https://dax.guide/datesytd/?aff=sqlbi). We chose those two functions for a good reason: they represent two different categories of time intelligence functions.

- [SAMEPERIODLASTYEAR](https://dax.guide/sameperiodlastyear/?aff=sqlbi) performs a lateral shift, meaning that it considers the current selection in the filter context and moves it back one year. If the original selection is the month level, [SAMEPERIODLASTYEAR](https://dax.guide/sameperiodlastyear/?aff=sqlbi) performs filters at the month level, just moving the filter one year back. A lateral shift does not change the granularity of the filter. The only other function that performs lateral shifts is [DATEADD](https://dax.guide/dateadd/?aff=sqlbi), which is the function internally used by [SAMEPERIODLASTYEAR](https://dax.guide/sameperiodlastyear/?aff=sqlbi).
- [DATESYTD](https://dax.guide/datesytd/?aff=sqlbi) performs a hierarchical shift. If the original filter contains only one date, the new filter may need to filter dates, years, and months. [DATESYTD](https://dax.guide/datesytd/?aff=sqlbi), as well as any time intelligence function other than [DATEADD](https://dax.guide/dateadd/?aff=sqlbi) and [SAMEPERIODLASTYEAR](https://dax.guide/sameperiodlastyear/?aff=sqlbi), can change the granularity of the selection and extend or reduce it. For example, [DATESYTD](https://dax.guide/datesytd/?aff=sqlbi) usually extends the period selected, whereas [NEXTMONTH](https://dax.guide/nextmonth/?aff=sqlbi) always returns one month, even if the initial selection was a quarter or a year.
