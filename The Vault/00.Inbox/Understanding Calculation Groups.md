...
created: 2026-08-04
description: "This article explores the properties of calculation groups in detail and then it describes how a calculation item is applied to a measure. Before starting,"
...
- The calculation item **Expression**. A DAX expression that might contain special functions like [SELECTEDMEASURE](https://dax.guide/selectedmeasure/?aff=sqlbi). This is the expression that defines how to apply the calculation item.
...
![](99.System/Attachments/Calculation-groups-2-Understanding-01.png)
...
Calculation items can be applied by the user using, for example, a slicer, or by using [CALCULATE](https://dax.guide/calculate/?aff=sqlbi) to filter the calculation group. For example, consider the following calculation item:
...
There is nothing magical about calculation groups: They are tables, and as such they can be filtered by [CALCULATE](https://dax.guide/calculate/?aff=sqlbi) like any other table. When [CALCULATE](https://dax.guide/calculate/?aff=sqlbi) applies a filter to a calculation item, DAX uses the definition of the calculation item to rewrite the expression before evaluating it.
...
When using client tools like Power BI, you never have to worry about these details. Indeed, these tools make sure that calculation items get applied the right way because they always invoke single measures as part of the query they execute. Nevertheless, as a DAX developer, you will end up using calculation items as filters in [CALCULATE](https://dax.guide/calculate/?aff=sqlbi). When you do that, pay attention to the expression used in [CALCULATE](https://dax.guide/calculate/?aff=sqlbi). If you want to stay on the safe side, use calculation items in [CALCULATE](https://dax.guide/calculate/?aff=sqlbi) to modify a single measure. Never apply calculation items to an expression.
...
The best practice for using calculation items in DAX code is to apply them as filters in [CALCULATE](https://dax.guide/calculate/?aff=sqlbi) statements that only evaluate a single measure reference. If you are curious about the reasons for this best practice, make sure you do not miss the next article about [how calculation items are applied to measure references.](https://www.sqlbi.com/articles/understanding-the-application-of-calculation-items/)
