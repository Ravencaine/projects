...
created: 2026-08-04
description: "This article shows how to create a DAX measure that displays information from multiple columns in a business entity or transaction, into a single column of"
...
![](99.System/Attachments/image1-124.png)
...
![](99.System/Attachments/image2-120.png)
...
![](99.System/Attachments/image3-108.png)
...
```
Details v0.A =
VARCurrencyCode =SELECTEDVALUE(Sales[Currency Code])
VARExchangeRate =SELECTEDVALUE(Sales[Exchange Rate])
RETURN
    IF(
        NOTISBLANK(CurrencyCode ),
...
![](99.System/Attachments/image4-102.png)

The first issue to address is that the information is also visible when multiple orders share the same currency code. This can be solved by ensuring that only one order is visible: using [ISINSCOPE](https://dax.guide/isinscope/?aff=sqlbi) guarantees that the *Order Number* is also displayed in the matrix visual, not just filtered outside of the visual. By checking that there is a row visible in *Sales*, we can also use [VALUES](https://dax.guide/values/?aff=sqlbi) instead of the more expensive [SELECTEDVALUE](https://dax.guide/selectedvalue/?aff=sqlbi), which relies on an underlying [DISTINCTCOUNT](https://dax.guide/distinctcount/?aff=sqlbi):
...
Details v0.B =
IF(
    ISINSCOPE(Sales[Order Number])&& NOTISEMPTY(Sales ),
    VARCurrencyCode =VALUES(Sales[Currency Code])
    VARExchangeRate =VALUES(Sales[Exchange Rate])
    RETURNCurrencyCode & "/USD="& ExchangeRate
)
```
...
IF(
    ...
    RETURN
        IF(
            NOTISBLANK(CurrencyCode )&& NOTISBLANK(ExchangeRate ),
            CurrencyCode & "/USD="& ExchangeRate
        )
)
```
...
![](99.System/Attachments/image5-87.png)

The code uses a list of values in a table constructor, which [CONCATENATEX](https://dax.guide/concatenatex/?aff=sqlbi) iterates over to produce the output string. [FILTER](https://dax.guide/filter/?aff=sqlbi) makes sure to remove empty values, so that the measure returns [BLANK](https://dax.guide/blank/?aff=sqlbi) if the list is empty (without the filter, we would always have a list of two blank values):
...
Having introduced the method of iterating over a list of values utilizing [CONCATENATEX](https://dax.guide/concatenatex/?aff=sqlbi) and [FILTER](https://dax.guide/filter/?aff=sqlbi), we can now tackle the subsequent challenge: presenting data from various tables, for instance, the *Customer* table.

To use the same approach as what we introduced in the previous example, we must propagate the filter from *Sales* to *Customer*; this is because the filter context only filters one order, but not the related customer. We can wrap the [CONCATENATEX](https://dax.guide/concatenatex/?aff=sqlbi) expression in a [CALCULATE](https://dax.guide/calculate/?aff=sqlbi) function where [CROSSFILTER](https://dax.guide/crossfilter/?aff=sqlbi) enables the filter propagation from *Sales* to *Customer*:
...
![](99.System/Attachments/image6-79.png)
...
![](99.System/Attachments/image7-67.png)
...
Because the *Customer* table must be filtered to return only the row corresponding to the selected order, we use [CALCULATETABLE](https://dax.guide/calculatetable/?aff=sqlbi) and [CROSSFILTER](https://dax.guide/crossfilter/?aff=sqlbi) to propagate the filter from *Sales* to *Customer*. Because we know a filter is active on *Sales* (thanks to the initial [ISINSCOPE](https://dax.guide/isinscope/?aff=sqlbi)), we can be confident that only one customer is filtered, even if the order has multiple rows in *Sales*.

The outer [CONCATENATEX](https://dax.guide/concatenatex/?aff=sqlbi) should only iterate one row in *Customer*. Therefore, the “ – “ separator should never be used. The result of the *Details V2.A* measure is identical to *Details V1*.

![](99.System/Attachments/image8-56.png)
...
![](99.System/Attachments/image9-48.png)
...
![](99.System/Attachments/image10-42.png)
...
The last version has the same structure as version 2, but in this case, we obtain a row context to use column references with [SUMMARIZE](https://dax.guide/summarize/?aff=sqlbi). This way, we do not have to rely on the bidirectional filter and we can also combine columns from different tables:
...
![](99.System/Attachments/image11-31.png)

To be honest, this solution is not ideal from a maintenance point of view, because it requires duplicating the list of columns both in [SUMMARIZE](https://dax.guide/summarize/?aff=sqlbi) and in the table constructor used as [FILTER](https://dax.guide/filter/?aff=sqlbi) argument. However, we wanted to show this approach because it could be more efficient from a query execution perspective.

![](99.System/Attachments/image12-22.png)
...
Local.ConcatenateNonEmpty =(
    fieldsListTable : TABLEVAL,
    fieldColumn : ANYREFEXPR
)=> 
CONCATENATEX(
    FILTER(
        fieldsListTable,
...
The measure reduces its verbosity by invoking the *Local.ConcatenateNonEmpty* function to implement the inner [CONCATENATEX](https://dax.guide/concatenatex/?aff=sqlbi) iteration:
