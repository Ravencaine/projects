...
source: "https://www.sqlbi.com/articles/using-removefilters-in-dax-user-defined-functions/?utm_source=powerbiweekly&utm_medium=email&utm_campaign=newsletter"
author:
  - "[[Marco Russo & Alberto Ferrari]]"
published: 2026-06-29
created: 2026-08-08
description: "In this article, we implement a function that removes filter-keep column filters from a calendar, using REMOVEFILTERS as the return value of the function. A"
Processed: "Unprocessed"
---
A DAX user-defined function, also known as a UDF, is expected to return a scalar or a table. However, because functions are fundamentally macro-expansion of DAX code, it is possible to return [CALCULATE](https://dax.guide/calculate/?aff=sqlbi) modifiers if the function is to be called only as a filter argument of [CALCULATE](https://dax.guide/calculate/?aff=sqlbi).
...
It is worth noting that we had to use -2 in [RANGE](https://dax.guide/range/?aff=sqlbi) rather than -3, because the current row is included in the range.
...
The function can work only when it is being used as a [CALCULATE](https://dax.guide/calculate/?aff=sqlbi) argument, as we do in the measure:
...
The function will not work if called differently, because its result is not a table; it is a [CALCULATE](https://dax.guide/calculate/?aff=sqlbi) modifier. However, as long as developers call the function from inside [CALCULATE](https://dax.guide/calculate/?aff=sqlbi), it works just fine.
...
An alternative, a very valid alternative, is to use an EXPR parameter and embed the [CALCULATE](https://dax.guide/calculate/?aff=sqlbi) call inside the function:
...
Functions use macro-expansion in DAX. This opens up the possibility of using code in functions that would not work as standalone code, but will work when executed in the proper environment. Specifically, in this article, we outlined how you can make a function “return” [REMOVEFILTERS](https://dax.guide/removefilters/?aff=sqlbi) if the function is being used in [CALCULATE](https://dax.guide/calculate/?aff=sqlbi) only.
