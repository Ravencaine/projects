...
created: 2026-08-04
description: "This article describes the sideways recursion triggered by invoking a calculation item from another calculation item, explaining why it should be avoided to"
...
The requirement is to add a third calculation item that computes the year-to-date in the previous year (**PYTD**). This can be obtained by mixing two time intelligence functions: [DATESYTD](https://dax.guide/datesytd/?aff=sqlbi) and [SAMEPERIODLASTYEAR](https://dax.guide/sameperiodlastyear/?aff=sqlbi). The following calculation item solves the scenario:
...
Sideways recursion can lead to very complex expressions that are hard to read and likely to produce unexpected results. Most of the complexity of calculation items with sideways recursion is seen when there are measures that internally apply calculation items with [CALCULATE](https://dax.guide/calculate/?aff=sqlbi) —all the while users change the calculation item through the user interface of the tool, like using a slicer in Power BI.
